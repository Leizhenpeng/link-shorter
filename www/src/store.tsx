...
export const handleShort = async (state: ILinkContext, alterState: IAlterContext, locale: SpeakLocale) => {
  const pass = ValidateUrl(state.rawUrl);
  const ifCn = locale.lang === "zh-CN";
  if (!pass) {
    state.ifError = true;
    showTip(alterState, "warning", ifCn?"不要胡闹, 请输入正确的链接":"Don't mess around, please enter the correct link",5000);
    return;
  }
  ...
  if (res.code !== 200) {
    state.ifError = true;
    showTip(alterState, "warning", ifCn? "你很调皮, 这个链接不支持":"You are naughty, this link is not supported" , 5000);
    return;
  }
  state.ifShowResult = true;
  state.shortUrl = composeShortUrl(res.data);
  showTip(alterState, "success", ifCn ? ("很开心, 转换成功啦") : "Very happy, the conversion is successful", 3000);
};
